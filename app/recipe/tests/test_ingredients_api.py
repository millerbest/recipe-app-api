...
class PrivateIngredientsApiTests(TestCase):
    ...
    def test_filter_ingredients_assigned_to_recipes(self):
        """Tset listing ingredients by those assigned to recipes"""
        in1 = Ingredient.objects.create(user=self.user, name="Apples")
        in2 = Ingredient.objects.create(user=self.user, name="Turkey")
        recipe = Recipe.objects.create(
            title="Apple crumble", time_minutes=5, price=Decimal(10.00), user=self.user
        )
    ...
    def test_filtered_ingredients_unique(self):
        ...
        recipe1 = Recipe.objects.create(
            title="Egg Benedict", time_minutes=5, price=Decimal(10.00), user=self.user
        )
        recipe2 = Recipe.objects.create(
            title="Herb Eggs", time_minutes=5, price=Decimal(10.00), user=self.user
        )
